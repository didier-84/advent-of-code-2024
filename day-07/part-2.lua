...
function run(input)
  ...
  for _, equation in ipairs(equations) do
    ...
    concat_next = function(numbers)
      local first, numbers = grid_helper.array_shift(numbers)

      numbers[1] = tonumber(tostring(first) .. tostring(numbers[1]))
    ...
    is_solvable = function(numbers)
      return add_next(numbers) or multiply_next(numbers) or concat_next(numbers)
    end
