...
function run(input)
  ...
  local function fix_update_order(update, rules)
    for i, number in ipairs(update) do
      for j = i+1, #update do
        local next = update[j]
        local wrong_order_key = tostring(next .. '|' .. number)

        if rules[wrong_order_key] == true then
          update[i], update[j] = next, number
          break
        end
      end
    end
    ...
    if validate_update_order(update, rules) then
      ...
      return fix_update_order(update, rules)
    end
  end

  for _, update in ipairs(updates) do
    if validate_update_order(update, rules) == false then
      update = fix_update_order(update, rules)
